class Backoffice::QuestionsController < Admin::BaseController
  ...
  if @question.save then redirect_to admin_theme_questions_path(@theme), notice: "Question créée"
  ...
  if @question.update(question_params) then redirect_to admin_theme_questions_path(@theme), notice: "Mis à jour"
  else render :edit, status: :unprocessable_entity
  end
  end
  def destroy
  @theme.questions.find(params[:id]).destroy
  redirect_to admin_theme_questions_path(@theme), notice: "Supprimée"
  end
  private
