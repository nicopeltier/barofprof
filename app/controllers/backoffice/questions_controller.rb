class Backoffice::QuestionsController < Backoffice::BaseController
  ...
  def create
    ...
    if @question.save then redirect_to backoffice_theme_path(@theme), notice: "Question créée"
  ...
  def update
    ...
    if @question.update(question_params) then redirect_to backoffice_theme_path(@theme), notice: "Mis à jour"
    else render :edit, status: :unprocessable_entity
    end
  end
  def destroy
    @theme.questions.find(params[:id]).destroy
    redirect_to backoffice_theme_path(@theme), notice: "Supprimée"
  end
  private
