class Backoffice::MessageTemplatesController < Admin::BaseController
  ...
  if @template.save then redirect_to admin_message_templates_path, notice: "Modèle créé"
  ...
  if @template.update(template_params) then redirect_to admin_message_templates_path, notice: "Mis à jour"
  else render :edit, status: :unprocessable_entity
  end
  end
  def destroy
  MessageTemplate.find(params[:id]).destroy
  redirect_to admin_message_templates_path, notice: "Supprimé"
  end
  private
  def template_params
